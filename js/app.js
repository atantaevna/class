...
function view() {
    ...
    tasks.map((el, idx) => {
        ...
        <div class="line">${el.name[0].toUpperCase()}${el.username[0].toUpperCase()}</div>  name: ${el.name} surname: ${el.username}  <button class="del-btn btn btn-danger">delete</button> </li>`
    })
    deleteTask()
    firstInput.value = ''
    secondInput.value = ''
...
btn.addEventListener('click', () => {
    if (firstInput.value !== '' && secondInput.value !== '') {
        ...
        const newTask = {
            ...
            name: firstInput.value, username: secondInput.value,
        }
        tasks = [...tasks, newTask]
